...
class XShell(Wox):
        ...
        for f in listdir(session_path):
            if isfile(join(session_path, f)) and not f.endswith("ini"):
                ...
                if session_path not in dirs:
                    files.append({'f': session_path.split(
                        '\\')[-1], 'd': session_path})
    ...
    def query(self, query):
        ...
        for p in sessions:
            if query in p['f']:
                res.append({"Title": p['f'].replace(".xsh", ""),
                            "IcoPath": "terminal.ico", "JsonRPCAction": {
                    "method": "open_session",
                    "parameters": [p['d'].replace("\\", "\\\\"), p['f']]}})
        return res
            ...
            for child in listdir(folder):
                ...
                if isfile(childSession) and not child.endswith("ini"):
                    subprocess.call('{}'.format(
                        childSession.encode('gbk')), shell=True)
